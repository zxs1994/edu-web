...
const formData = ref<Partial<TravelApplyBillApi.TravelApplyBill>>({});
...
async function handleSaveAndSubmit(isSubmit: boolean) {
  ...
  try {
    const formValues = isSubmit
      ? ((await basicFormRef.value.getFormValues()) as TravelApplyBillApi.TravelApplyBill)
      : ((await basicFormRef.value.getFormValues(
          false,
        )) as TravelApplyBillApi.TravelApplyBill);
    ...
    id = await (isSubmit
      ? submitTravelApplyBill(data)
      : saveTravelApplyBill(data));
...
async function handleDelete() {
  ...
  loading.value = true;
  try {
    await deleteTravelApplyBill(id);
...
async function loadData() {
  ...
  loading.value = true;
  try {
    const data = await getTravelApplyBill(id);
    formData.value = { ...data };
    readonly.value =
